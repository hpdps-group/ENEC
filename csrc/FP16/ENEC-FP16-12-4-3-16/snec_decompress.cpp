...
public:
    ...
    )
    {
        ...
        int FinalOtherSize = 32 + // 头
                            ...
                            (datablockSize / sizeof(uint16_t)) * 3 / 8 * datablockNum + // low bits
...
private:
                                ...
                                )
    {
        ...
        if(accouterNum + outerNum >= computeNum)
        {
            ...
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16].template ReinterpretCast<int16_t>(), mergeLocal[accouterNum].template ReinterpretCast<int16_t>(), (int16_t)12, remainNum);
            ...
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + remainNum].template ReinterpretCast<int16_t>(), mergeLocal.template ReinterpretCast<int16_t>(), (int16_t)12, nextreadNum);
            accouterNum  = nextreadNum;
        }
        else {
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16].template ReinterpretCast<int16_t>(), mergeLocal[accouterNum].template ReinterpretCast<int16_t>(), (int16_t)12, outerNum);
            // if(i == 0){
    ...
    //     AIV_WITH_BARRIER(ShiftRight, tempLocal0[computeNum], tempLocal0[computeNum], (uint16_t)13, computeNum);
        ...
        // }
        AIV_WITH_BARRIER(Adds, tempLocal0[computeNum].template ReinterpretCast<int16_t>(), tempLocal0[computeNum].template ReinterpretCast<int16_t>(), (int16_t)12, computeNum);

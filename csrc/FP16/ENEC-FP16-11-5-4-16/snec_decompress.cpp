...
public:
    ...
    )
    {
        ...
        int FinalOtherSize = 32 + // 头
                            ...
                            (datablockSize / sizeof(uint16_t)) * 11 / 8 * datablockNum + // ms1
                            (datablockSize / sizeof(uint16_t)) * 4 / 8 * datablockNum + // low bits
                            tileNum / 8 * datablockNum + // mbl compareMask
                            BLOCK_NUM * 4; // prefix

        this->threadcompedNum = ((blockId == BLOCK_NUM - 1 ? totalCompressedBytes - FinalOtherSize : compPrefixLocal(blockId + 1)) - compPrefixLocal(blockId)) * 8 / 1;
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        AIV_WITH_BARRIER(DataCopy, mergeLocal, e_input1, computeNum * 1 / 16);
        ...
        AIV_WITH_BARRIER(ShiftRight, mergeLocal[computeNum0 * 1 / 16], mergeLocal, (uint16_t)8, computeNum0 * 1 / 16);
        AIV_WITH_BARRIER(ShiftRight, mergeLocal[computeNum0 * 1 / 16 * 2], mergeLocal, (uint16_t)4, computeNum0 * 1 / 16 * 2);
        AIV_WITH_BARRIER(ShiftRight, mergeLocal[computeNum0 * 1 / 16 * 4], mergeLocal, (uint16_t)2, computeNum0 * 1 / 16 * 4);
        AIV_WITH_BARRIER(ShiftRight, mergeLocal[computeNum0 * 1 / 16 * 8], mergeLocal, (uint16_t)1, computeNum0 * 1 / 16 * 8);
        AIV_WITH_BARRIER(ShiftLeft, mergeLocal, mergeLocal, (uint16_t)15, computeNum0);

        // AIV_WITH_BARRIER(ShiftRight, mergeLocal[computeNum0 * 3 / 16], mergeLocal, (uint16_t)8, computeNum0 * 3 / 16);
        ...
        // AIV_WITH_BARRIER(ShiftRight, mergeLocal[computeNum0 / 2], mergeLocal, (uint16_t)3, computeNum0 / 2);

        // AIV_WITH_BARRIER(ShiftLeft, mergeLocal, mergeLocal, (uint16_t)13, computeNum0);
...
private:
                                ...
                                )
    {
        ...
        if(accouterNum + outerNum >= computeNum)
        {
            ...
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16].template ReinterpretCast<int16_t>(), mergeLocal[accouterNum].template ReinterpretCast<int16_t>(), (int16_t)11, remainNum);

            uint32_t computeNum0 = accCompressed + computeNum >= threadcompedNum ? threadcompedNum - accCompressed : computeNum;

            AIV_WITH_BARRIER(DataCopy, mergeLocal, e_input1[accCompressed  * 1 / 8 / sizeof(T)], computeNum0 * 1 / 8 / sizeof(T));
            ...
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + remainNum].template ReinterpretCast<int16_t>(), mergeLocal.template ReinterpretCast<int16_t>(), (int16_t)11, nextreadNum);
            accouterNum  = nextreadNum;
        }
        else {
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16].template ReinterpretCast<int16_t>(), mergeLocal[accouterNum].template ReinterpretCast<int16_t>(), (int16_t)11, outerNum);
            // if(i == 0){
        ...
        // if(i == 0){
        //     DumpTensor(tempLocal0[computeNum], 1, 512);
        // }
        ...
        AIV_WITH_BARRIER(ShiftRight, tempLocal0[computeNum][computeNum / 4], tempLocal0[computeNum], (uint16_t)8, computeNum / 4);
    ...
    //     AIV_WITH_BARRIER(ShiftRight, tempLocal0[computeNum], tempLocal0[computeNum], (uint16_t)13, computeNum);
        ...
        // if(i == 0){
        //     DumpTensor(tempLocal0[computeNum - 512], 1, 512);
        // }
        // if(i == 0){
        ...
        // }
        AIV_WITH_BARRIER(Adds, tempLocal0[computeNum].template ReinterpretCast<int16_t>(), tempLocal0[computeNum].template ReinterpretCast<int16_t>(), (int16_t)11, computeNum);

        // if(i == 0){
        //     DumpTensor(tempLocal0[computeNum].template ReinterpretCast<int16_t>(), 1, 512);
        // }
        ...
        // 恢复原始数据，尾数，符号，指数进行组合
        AIV_WITH_BARRIER(DataCopy, tempLocal1[16], ms_input1[computeNum / sizeof(uint16_t) * 11 / 8 * i], computeNum / sizeof(uint16_t) * 11 / 8);
        // if(i == 0){
        //     DumpTensor(tempLocal1[16].template ReinterpretCast<uint16_t>()
        //     // [computeNum - 512]
        //     , 1, 512);
        // }
        AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + (computeNum + computeNum / 4 + computeNum / 8) / 2], tempLocal1[16], (uint16_t)8, (computeNum + computeNum / 4 +computeNum / 8) / 2);
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1[16], tempLocal1[16], (uint16_t)8, (computeNum + computeNum / 4 + computeNum / 8) / 2);
        AIV_WITH_BARRIER(ShiftRight, tempLocal1[16], tempLocal1[16], (uint16_t)8, (computeNum + computeNum / 4 + computeNum / 8) / 2);
        // if(i == 0){
        //     DumpTensor(tempLocal1[16].template ReinterpretCast<uint16_t>()
        //     // [computeNum - 512]
        //     , 1, 512);
        // }
        AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + computeNum + computeNum / 4 + computeNum / 8], tempLocal1[16 + computeNum + computeNum / 4], (uint16_t)4, computeNum / 8);
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1[16 + computeNum + computeNum / 4], tempLocal1[16 + computeNum + computeNum / 4], (uint16_t)8, computeNum / 4);
        AIV_WITH_BARRIER(Or, tempLocal1[16 + computeNum], tempLocal1[16 + computeNum], tempLocal1[16 + computeNum + computeNum / 4], computeNum / 4);
        // if(i == 0){
        //     DumpTensor(tempLocal1[16].template ReinterpretCast<uint16_t>()
        //     // [computeNum - 512]
        //     , 1, 512);
        // }
        AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + computeNum + computeNum / 4], tempLocal1[16 + computeNum], (uint16_t)6, computeNum / 4);
        AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + computeNum + computeNum / 2], tempLocal1[16 + computeNum], (uint16_t)3, computeNum / 2);
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1[16 + computeNum], tempLocal1[16 + computeNum], (uint16_t)13, computeNum);
        AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + computeNum], tempLocal1[16 + computeNum], (uint16_t)5, computeNum);

        AIV_WITH_BARRIER(Or, tempLocal1[16], tempLocal1[16], tempLocal1[16 + computeNum], computeNum);
        // AIV_WITH_BARRIER(ShiftLeft, tempLocal1[16], tempLocal1[16], (uint16_t)5, computeNum);
 
        // if(i == 0){
        //     DumpTensor(tempLocal1[16]
        //         , 1, 512);
        // }
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1[16], tempLocal1[16], (uint16_t)5, computeNum);
        ...
        AIV_WITH_BARRIER(Or, tempLocal0[computeNum], tempLocal0[computeNum], tempLocal1[16], computeNum);
        // if(i == 0){
    ...
    //     // AIV_WITH_BARRIER(Or, tempLocal0[computeNum], tempLocal0[computeNum], tempLocal0, computeNum);
        ...
        AIV_WITH_BARRIER(ShiftRight, tempLocal0, tempLocal0[computeNum], (uint16_t)6, computeNum);
        AIV_WITH_BARRIER(ShiftLeft, tempLocal0[computeNum], tempLocal0[computeNum], (uint16_t)10, computeNum);
        AIV_WITH_BARRIER(Or, tempLocal0[computeNum], tempLocal0[computeNum], tempLocal0, computeNum);
        // if(i == 0){
        //     DumpTensor(tempLocal0[computeNum].template ReinterpretCast<uint16_t>(), 1, 512);
        // }

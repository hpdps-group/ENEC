...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        AIV_WITH_BARRIER(Duplicate, mask7Local, (T)15, 32 / sizeof(T));
...
private:
    ...
    {   // tempLocal0: e_inLocal(前半), mergeLocal(后半)
        ...
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1, tempLocal0, (uint16_t)12, computeNum);
        AIV_WITH_BARRIER(ShiftRight, tempLocal1, tempLocal1, (uint16_t)12, computeNum);
        // if(i == 0){
        //     DumpTensor(tempLocal1.template ReinterpretCast<uint16_t>(), 1, 512);
        // }
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1[computeNum / 2], tempLocal1[computeNum / 2], (uint16_t)4, computeNum / 2);
        AIV_WITH_BARRIER(Or, tempLocal1, tempLocal1, tempLocal1[computeNum / 2], computeNum / 2);
        AIV_WITH_BARRIER(ShiftLeft, tempLocal1[computeNum / 4], tempLocal1[computeNum / 4], (uint16_t)8, computeNum / 4);
        AIV_WITH_BARRIER(Or, tempLocal1, tempLocal1, tempLocal1[computeNum / 4], computeNum / 4);
        ...
        AIV_WITH_BARRIER(DataCopy, e_output0[i * (computeNum * 4 / 16)], tempLocal1, computeNum * 4 / 16);
        ...
        if(cumulated_amount + outerNum >= computeNum){
            ...
            AIV_WITH_BARRIER(ShiftRight, mergeLocal[cumulated_amount], tempLocal1, (uint16_t)4, low_write_num);
            // if(i == 0){
            ...
            AIV_WITH_BARRIER(ShiftRight, mergeLocal, tempLocal1[low_write_num], (uint16_t)4, high_unwrite_num);
            cumulated_amount = 
            high_unwrite_num;
            // assert(cumulated_amount % 16 == 0);
        }
        else {
            AIV_WITH_BARRIER(ShiftRight, mergeLocal[cumulated_amount], tempLocal1, (uint16_t)4, outerNum);
            // ,
